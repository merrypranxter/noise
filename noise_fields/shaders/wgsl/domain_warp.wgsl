...
// Domain warp utility: distort coordinates using noise.
...
fn domainWarpIterative2D(p: vec2<f32>, warpStrength: f32, warpFreq: f32, iterations: i32) -> vec2<f32> {
    var q = p;
...
fn domainWarpIterative3D(p: vec3<f32>, warpStrength: f32, warpFreq: f32, iterations: i32) -> vec3<f32> {
    var q = p;
    for (var i = 0; i < iterations; i++) {
